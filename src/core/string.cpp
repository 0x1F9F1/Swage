#include "string.h"

namespace Swage
{
    // clang-format off
    const u8 NormalizeCaseAndSlash[256] {
        
    };
    // clang-format on

    String Concat(std::initializer_list<StringView> values)
    {
        String result;

        usize total = 0;

        for (StringView v : values)
            total += v.size();

        result.reserve(total);

        for (StringView v : values)
            result += v;

        return result;
    }

    bool StartsWith(StringView haystack, StringView needle)
    {
        return haystack.size() >= needle.size() && haystack.compare(0, needle.size(), needle) == 0;
    }

    bool EndsWith(StringView haystack, StringView needle)
    {
        return haystack.size() >= needle.size() &&
            haystack.compare(haystack.size() - needle.size(), needle.size(), needle) == 0;
    }

    bool StartsWithI(StringView haystack, StringView needle)
    {
        return haystack.size() >= needle.size() && StringICompareEqual(haystack.substr(0, needle.size()), needle);
    }

    bool EndsWithI(StringView haystack, StringView needle)
    {
        return haystack.size() >= needle.size() &&
            StringICompareEqual(haystack.substr(haystack.size() - needle.size(), needle.size()), needle);
    }

    bool StringICompareEqual(StringView lhs, StringView rhs)
    {
        usize len = lhs.size();

        if (len != rhs.size())
            return false;

        const char* const lhs_raw = lhs.data();
        const char* const rhs_raw = rhs.data();

        for (usize i = 0; i != len; ++i)
        {
            unsigned char const i1 = static_cast<unsigned char>(lhs_raw[i]);
            unsigned char const i2 = static_cast<unsigned char>(rhs_raw[i]) ^ i1;

            if (i2 && (i2 != 0x20 || (static_cast<unsigned char>((i1 | 0x20) - 0x61) >= 26)))
                return false;
        }

        return true;
    }

    bool StringICompareLess(StringView lhs, StringView rhs)
    {
        usize const l1 = lhs.size();
        usize const l2 = rhs.size();

        for (usize i = 0, len = std::min(l1, l2); i < len; ++i)
        {
            char const i1 = ToLower(lhs[i]);
            char const i2 = ToLower(rhs[i]);

            if (i1 != i2)
                return i1 < i2;
        }

        return l1 < l2;
    }

    bool PathCompareEqual(StringView lhs, StringView rhs)
    {
        usize len = lhs.size();

        if (len != rhs.size())
            return false;

        const char* const lhs_raw = lhs.data();
        const char* const rhs_raw = rhs.data();

        for (usize i = 0; i != len; ++i)
        {
            if (NormalizeCaseAndSlash[static_cast<unsigned char>(lhs_raw[i])] !=
                NormalizeCaseAndSlash[static_cast<unsigned char>(rhs_raw[i])])
                return false;
        }

        return true;
    }

    bool PathCompareLess(StringView lhs, StringView rhs)
    {
        usize const l1 = lhs.size();
        usize const l2 = rhs.size();

        const char* const lhs_raw = lhs.data();
        const char* const rhs_raw = rhs.data();

        usize const len = std::min(l1, l2);

        for (usize i = 0; i < len; ++i)
        {
            u8 const i1 = NormalizeCaseAndSlash[static_cast<unsigned char>(lhs_raw[i])];
            u8 const i2 = NormalizeCaseAndSlash[static_cast<unsigned char>(rhs_raw[i])];

            if (i1 != i2)
                return i1 < i2;
        }

        return l1 < l2;
    }
} // namespace Swage

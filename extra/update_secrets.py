...
def hash_poly(data, poly):
    result = 0

    for v in data:
        result = ((result + v) * poly) & 0xFFFFFFFF
...
def CRC64(data, crc = 0):
    ...
    for v in data:
        crc = (crc >> 8) ^ CRC64Table[(crc ^ v) & 0xFF]

    return ~crc & 0xFFFFFFFFFFFFFFFF

def old_secret_hash(data):
    length = len(data)
    h1 = hash_poly(data, 0x9E3779B1)
    h2 = CRC64(data)
    h85 = base64.b85encode(struct.pack('<IIQ', length, h1, h2))
    return h85

def new_secret_hash(data):
    length = len(data)
    h1 = hash_poly(data, 0x9E3779B1)
    h2 = CRC64(data)
    h85 = base64.b85encode(struct.pack('<IIQ', length, h1, h2))
...
with open('secrets.bin', 'rb') as f:
    while True:
        ...
        all_secrets[old_secret_hash(data)] = data
...
def replace_hash(match):
    ...
    if h in all_secrets:
        h = new_secret_hash(all_secrets[h])
    else:
        print('Skipped', h)
...
for file in files:
    ...
    with open(file, 'r+b') as f:
        file_data = f.read()
        file_data = re.sub(rb'"([0-9A-Za-z!#$%&()*+\-;<=>?@^_`{|}~]{20})"', replace_hash, file_data)
